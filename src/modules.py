...
class UNet_conditional(nn.Module):
    ...
    def __init__(self,
        ...
        # Changed Conv2d to Conv1d for 1D output generation
        self.outc = nn.Conv1d(64, c_out, kernel_size=1)
        # self.outc = nn.Conv2d(64, c_out, kernel_size=1)

        self.label_prep = nn.Sequential(
            # TODO this might be a bad idea if the batchsize is small (which it is)
           nn.BatchNorm1d(feat_num),
           nn.Linear(feat_num, time_dim),
           nn.SiLU(),
        )

        # Replaced label_prep with nn.Embedding for class condition
