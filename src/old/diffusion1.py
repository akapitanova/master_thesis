...
class EdmSampler:
    ...
    def sample(self, length, device, class_labels=None, n_samples=1, cfg_scale=3, label_dim=0):
        """
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            with torch.no_grad():
                ...
                denoised = None
                if label_dim != 0: 
                    denoised_cond = self.net(x_hat, t_hat, class_labels).to(torch.float32)
            ...
            if i < self.num_steps - 1:
                with torch.no_grad():
                    ...
                    denoised = None
                    if label_dim != 0: 
                        denoised_cond = self.net(x_next, t_next, class_labels).to(torch.float32)
                        denoised = denoised_uncond + cfg_scale * (denoised_cond - denoised_uncond)
                    else:
                        denoised = denoised_uncond
